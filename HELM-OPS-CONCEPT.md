...
**Symptom:** Sync workflow creates empty PR

**Causes:**
- Upstream changed structure
- Sync script filters too aggressive
- Submodule not updated

**Resolution:**
```bash
# Manually inspect upstream
cd upstream/source-repo
git log --oneline -10

# Test sync locally
./scripts/sync.sh

# Update filters in sync.sh if needed
```

### Image Replication Failures

**Symptom:** Images fail to push to target registry
...
**Symptom:** Chart not appearing in main registry

**Causes:**
- PAT_TOKEN expired or lacks permissions
- Main registry workflow disabled
- GHCR push failed

**Resolution:**
1. Verify PAT_TOKEN has `repo` and `write:packages` scopes
2. Check main registry workflow runs
3. Manually trigger registry update
...
- [ ] Chart deprecation workflow

---

## References

- **Main Registry:** [helm-charts](https://github.com/chronolite-technologies/helm-charts)
- **Chart Repositories:** [chronolite-technologies](https://github.com/chronolite-technologies)
- **Contact:** [emil.schilberg@chronolite.tech](mailto:emil.schilberg@chronolite.tech)
